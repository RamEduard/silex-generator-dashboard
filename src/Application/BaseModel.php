...
class BaseModel
{
    ...
    /**
     * @var int Filas afectadas en el sql
     */
    protected $_affectedRows = 0;
    
    /**
     * @var string Alias de la tabla del modelo 
     */
    protected $_alias;

    /**
     * @var Connection Objeto de la conección a base de datos de Silex
     */
    ...
    /**
     * @var string SQL que se ejecuta al final de armarlo
     */
    protected $_sql = "";
    ...
    /**
     * @var int Last insert id de la tabla o id del registro
     */
    public $id;

    /**
     * @param Connection $connection Objeto de la conección de doctrine con la base de datos
    ...
    public function __construct(Connection $connection, $table = "")
    {
        if (!is_null($connection)) {
            ...
            $this->_queryBuilder = $this->_connection->createQueryBuilder();
        }
        if (!is_null($table)) {
            $this->_table = $table;
        }
    ...
    protected function _delete(array $criteria = array())
    {
        if (!is_null($this->_table)) {
            $this->_affectedRows = $this->_connection->delete($this->_table, $criteria);
    ...
    {
        if (!is_null($this->_table)) {
            $this->_affectedRows = $this->_connection->insert($this->_table, $data);
    ...
    {
        if (!is_null($this->_table)) {
            $this->_affectedRows = $this->_connection->update($this->_table, $data, $criteria);
            return $this->_affectedRows;
        }
    }
